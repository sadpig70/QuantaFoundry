...
def gen_spec():
    header = ("# rm15_encoder_t2 — [[15,1,3]] punctured Reed-Muller **완전 논리-입력 인코더**"
              "(입력 |x⟩=q14): 15-to-1 magic 증류 정준 기판, 거리-3 transversal-T 코드. "
              "Tier-2 CLIFFORD(정준 stabilizer tableau, dense 미사용). plan=H·CNOT 38게이트, "
              "no MatrixGate. 코드-정확성 witness=rm15_observe(심볼릭 안정군 역전파, 오라클 독립).\n")
    return (header +
            "```python id=bloq\n" + _bloq_code(15, rm15_gates()) + "\n```\n"
            '```json id=meta\n'
            '{"id": "rm15_encoder_t2", "n_sys": 15, "n_anc": 0, "tier": "clifford"}\n'
            "```\n")


def main():
    name = "rm15_encoder_t2"
