...
def main():
    ...
    # 3aar. TrackHE9 P5: code switching well-formedness (Steane↔[[8,3,2]] closed-negative + RM15 redirect, seal 아님)
    rc, out = run(["scripts/code_switch_observe.py", "--quick"])
    result["steps"]["code_switch_observe"] = {
        "rc": rc, "all_ok": "all_ok=True" in out,
        "pass": rc == 0 and "all_ok=True" in out}
    ...
    # 3aav. TrackHE9 P1 심화: 제10 경로 monomial 비대각 위상회로 커버 확장 (회로-이데알 elimination, seal 아님)
    rc, out = run(["scripts/groebner_monomial_observe.py", "--quick"])
    result["steps"]["groebner_monomial_observe"] = {
        "rc": rc, "all_ok": "all_ok=True" in out,
        "pass": rc == 0 and "all_ok=True" in out}
    ...
    # 3abb. TrackHE10 P5: surface-code lattice surgery merge/split logical CNOT witness (code switching 상보, seal 아님)
    rc, out = run(["scripts/lattice_surgery_observe.py", "--quick"])
    result["steps"]["lattice_surgery_observe"] = {
        "rc": rc, "all_ok": "all_ok=True" in out,
        "pass": rc == 0 and "all_ok=True" in out}

    # 3abc. TrackHE11 P2: PSL(2,7) ambivalent 판정 → ℚ(√−7) 복소 필연 선검증 (A₅ 허수쌍, seal 아님)
    rc, out = run(["scripts/psl27_observe.py", "--quick"])
    result["steps"]["psl27_observe"] = {
        "rc": rc, "all_ok": "all_ok=True" in out,
        "pass": rc == 0 and "all_ok=True" in out}

    # 3abd. TrackHE11 P1: A₅ Fourier ℚ(√5) 실현가능성 (Frobenius-Schur) 선검증 (√5 실봉인 준비, seal 아님)
    rc, out = run(["scripts/a5_fourier_observe.py", "--quick"])
    result["steps"]["a5_fourier_observe"] = {
        "rc": rc, "all_ok": "all_ok=True" in out,
        "pass": rc == 0 and "all_ok=True" in out}

    # 3abe. TrackHE11 P4: qutrit(d=3) 맥락성 — Yu-Oh 13 자체정정 + KCBS pentagon witness (seal 아님)
    rc, out = run(["scripts/ks_qutrit_observe.py", "--quick"])
    result["steps"]["ks_qutrit_observe"] = {
        "rc": rc, "all_ok": "all_ok=True" in out,
        "pass": rc == 0 and "all_ok=True" in out}
