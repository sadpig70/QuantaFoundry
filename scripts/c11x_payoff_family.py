# -*- coding: utf-8 -*-
"""
c11x_payoff_family.py - W12.18 C11xPayoffFamily.

Seal the remaining N=1285 payoff multipliers unlocked by c11x.

Sealed outputs:
  Tier-0 apps: cmul{4,16,256}_mod1285

Honesty boundary:
  - controlled multipliers are Tier-0 exact permutation app seals.
  - cmul2_mod1285 was sealed in W12.17 and is checked as an existing child.
...
def main() -> int:
    ...
    print("=" * 84)
    print("W12.18 C11xPayoffFamily - remaining N=1285 payoff multipliers.")
    print("Honesty: cmul powers=Tier-0 EXACT, full Shor-1285 deferred.")
    print("=" * 84)
    ...
    for mul in NEW_POWERS:
        ...
        cmuls.append(rec)
        print(
            f"[Cmul] {rec['id']:16} sealed={rec['sealed']} tier={rec['tier']} "
            f"gates={rec['gate_count']} maxc={rec['max_control']} c11={rec['control_dist'].get(11, 0)} "
    ...
    total_c11 = sum(row["control_dist"].get(11, 0) for row in cmuls)

    report = {
        "phase": "W12.18 C11xPayoffFamily",
        "honesty": "N=1285=5*257 payoff family completion. New cmul powers are Tier-0 exact "
                   "permutation app seals; full Shor-1285 remains deferred.",
        ...
        "family_seals": family,
        "totals": {"new_gate_count": total_gates, "new_c11_count": total_c11},
    }
    all_ok = (
        len(cmuls) == 3
        and all(row["sealed"] and row["tier"] == 0 and row["max_control"] == 11 and "c11x" in row["deps"]
                for row in cmuls)
        and indep_ok == len(indep)
        and all(family.values())
    )
    report["all_ok"] = bool(all_ok)
    out_path = os.path.join(OUT, "C11X-PAYOFF-1285-REPORT.json")
    json.dump(report, open(out_path, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
    print("-" * 84)
    print(f"all_ok={all_ok} -> .pgf/arith/C11X-PAYOFF-1285-REPORT.json")
