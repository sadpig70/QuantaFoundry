# -*- coding: utf-8 -*-
"""
shor635_frontier.py - W12.15 Shor635StructuralFrontier.

Lift the c10x payoff into a Shor-style structural app:
N=635=5*127, a=2, counting t=8, work=10.

Sealed outputs:
  Tier-0 apps: cmul{2,4,16,131,256}_mod635
  Tier-1 app:  shor635 (18q structural composition)

Honesty boundary:
  - controlled multipliers are Tier-0 exact permutation app seals.
  - shor635 is Tier-1 STRUCTURAL; no dense whole-unitary equivalence is claimed.
...
def _shor635_spec() -> tuple[str, int]:
    ...
    header = (
        f"# shor635 - Shor period-finding structural frontier (N={N}={'*'.join(map(str, _factor(N)))}, a={A}). "
        f"{n_sys}q: counting={T}, work={WORK}. "
        f"H^{T} controlled-U^(2^j)(powa={POWA}) iqft8. "
        "18q>EXACT_BOUND -> Tier-1 STRUCTURAL only. "
        "Readout illustrative: ord_635(2)=28 -> gcd(2^14+-1,635)=5,127.\n"
    )
    return (
        header
        + "```json id=app_meta\n"
        + json.dumps({"id": "shor635", "n_sys": n_sys, "n_anc": 0})
        + "\n```\n"
        + "```json id=plan\n"
        + json.dumps(plan)
        + "\n```\n"
...
def _assemble_shor635() -> dict:
    spec, n_sys = _shor635_spec()
    spec_path = os.path.join(SPECS_APPS, "shor635.app.pg")
    with open(spec_path, "w", encoding="utf-8", newline="\n") as f:
        f.write(spec)
    ...
    sealed_out = aa._seal_dict("shor635", n_sys, "C1-C4(structural)", u_hash, resource, 1)
    aa.Registry(APPREG)._admit(sealed_out)
    disk = json.load(open(os.path.join(APPREG, "shor635.sealed.json"), encoding="utf-8"))
    return {
        "id": "shor635",
        "n_sys": n_sys,
        "sealed": bool(disk.get("sealed")),
...
def main() -> int:
    ...
    print("=" * 84)
    print("W12.15 Shor635StructuralFrontier - N=635=5*127, c10x-enabled Shor frontier.")
    print("Honesty: cmul powers=Tier-0 EXACT, shor635=Tier-1 STRUCTURAL, readout=illustrative.")
    print("=" * 84)
    ...
    for mul in UNIQUE_POWERS:
        ...
        cmuls.append(rec)
        print(
            f"[Cmul] {rec['id']:15} sealed={rec['sealed']} tier={rec['tier']} "
            f"gates={rec['gate_count']} maxc={rec['max_control']} c10={rec['control_dist'].get(10, 0)} "
    ...
    shor = _assemble_shor635()
    print(
        f"[Shor635] sealed={shor['sealed']} tier={shor['tier']} n_sys={shor['n_sys']} "
    ...
    report = {
        "phase": "W12.15 Shor635StructuralFrontier",
        "honesty": "N=635=5*127 is the c10x payoff Shor structural frontier. "
                   "cmul powers are Tier-0 exact permutation app seals; shor635 is Tier-1 structural, "
    ...
    all_ok = (
        all(row["sealed"] and row["tier"] == 0 and row["max_control"] == 10 and "c10x" in row["deps"] for row in cmuls)
    ...
    out_path = os.path.join(OUT, "SHOR-FRONTIER-635-REPORT.json")
    json.dump(report, open(out_path, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
    print("-" * 84)
    print(f"all_ok={all_ok} -> .pgf/arith/SHOR-FRONTIER-635-REPORT.json")
