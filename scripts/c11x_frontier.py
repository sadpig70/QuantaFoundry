# -*- coding: utf-8 -*-
"""
c11x_frontier.py - W12.17 C11xPrimitiveFrontier.

Close the 11-control primitive gap for 11-work-bit modular arithmetic.

Sealed outputs:
  Tier-0 module: c11x
  Tier-0 app:    cmul2_mod1285 (N=1285=5*257, 12q, uses c11x)

Honesty boundary:
  - c11x is an exact 12q MultiControlX module.
  - cmul2_mod1285 is an exact controlled modular multiplier app sealed through a
    full-basis permutation C-app check.
  - Full Shor-1285 is not claimed here.
"""
...
def _c11x_spec() -> str:
    return (
        ...
        "bloq = MultiControlX(cvs=(1,) * 11)\n"
        "```\n"
        "```python id=golden\n"
        "import numpy as np\n"
        "# c11x: control=first 11 qubits (MSB), target=last (big-endian).\n"
        "golden = np.zeros((1<<12, 1<<12), dtype=complex)\n"
        "for s in range(1<<12):\n"
        "    o = (s ^ 1) if (s >> 1) == ((1<<11) - 1) else s\n"
        "    golden[o, s] = 1\n"
        "```\n"
        "```json id=meta\n"
        "{\"id\": \"c11x\", \"n_sys\": 12, \"n_anc\": 0}\n"
        "```\n"
    )
...
def _seal_c11x() -> dict:
    ...
    spec_path = os.path.join(SPECS_MODS, "c11x.pg")
    with open(spec_path, "w", encoding="utf-8", newline="\n") as f:
        f.write(_c11x_spec())
    seal_path = os.path.join(MODREG, "c11x.sealed.json")
    if not os.path.exists(seal_path):
        ...
        if proc.returncode != 0 and not os.path.exists(seal_path):
            return {"id": "c11x", "sealed": False, "stderr": proc.stderr[-1600:], "stdout": proc.stdout[-1600:]}
...
def _seal_cmul1285() -> dict:
    ...
    if maxc != 11 or "c11x" not in deps:
        raise RuntimeError(f"cmul{A}_mod{N} did not exercise c11x: deps={deps}, maxc={maxc}")
...
def main() -> int:
    ...
    print("=" * 84)
    print("W12.17 C11xPrimitiveFrontier - c11x + cmul2_mod1285.")
    print("Honesty: c11x/cmul2_mod1285=Tier-0 EXACT, full Shor-1285 deferred.")
    print("=" * 84)
    print(f"[Target] N={N} factors={_factor(N)} a={A} WORK={WORK} nq={NQ}")

    c11x = _seal_c11x()
    if not c11x.get("sealed"):
        print(f"[C11X] sealed=False stderr={c11x.get('stderr', '')[-500:]}")
        report = {
            "phase": "W12.17 C11xPrimitiveFrontier",
            "honesty": "Attempted c11x seal, but oracle did not emit a seal. No payoff app was claimed.",
            "target": {"N": N, "a": A, "factors": _factor(N), "work": WORK, "nq": NQ},
            "c11x_module": c11x,
            "all_ok": False,
        }
        out_path = os.path.join(OUT, "C11X-FRONTIER-1285-REPORT.json")
        json.dump(report, open(out_path, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
        return 1

    c11x_indep = _independent_cnx_hash(11)
    c11x_match = c11x.get("u_hash") == c11x_indep
    print(f"[C11X] sealed={c11x['sealed']} tier={c11x['tier']} n_sys={c11x['n_sys']} match={c11x_match}")

    cmul = _seal_cmul1285()
    print(
        f"[Cmul] {cmul['id']} sealed={cmul['sealed']} tier={cmul['tier']} "
        f"gates={cmul['gate_count']} maxc={cmul['max_control']} c11={cmul['control_dist'].get(11, 0)}"
    )
    ...
    report = {
        "phase": "W12.17 C11xPrimitiveFrontier",
        "honesty": "c11x closes the 11-control primitive gap for 11-work-bit modular arithmetic. "
                   "c11x and cmul2_mod1285 are Tier-0 exact seals; full Shor-1285 is deferred.",
        "target": {"N": N, "a": A, "factors": _factor(N), "work": WORK, "nq": NQ},
        "c11x_module": c11x,
        "c11x_independent": {"independent_u_hash": c11x_indep[:16], "matches_sealed": c11x_match},
        "cmul_app": cmul,
        "independent_arith_verify": cmul_indep,
        "period_readout_illustrative": readout,
    }
    all_ok = (
        c11x["sealed"] and c11x.get("tier") == 0 and c11x.get("n_sys") == 12 and c11x_match
        and cmul["sealed"] and cmul["tier"] == 0 and cmul["max_control"] == 11
        and "c11x" in cmul["deps"] and cmul_indep["matches_sealed"]
    ...
    json.dump(report, open(out_path, "w", encoding="utf-8"), ensure_ascii=False, indent=2)
    print("-" * 84)
    print(f"all_ok={all_ok} -> .pgf/arith/C11X-FRONTIER-1285-REPORT.json")
