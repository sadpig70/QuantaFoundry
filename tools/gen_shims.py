...
if __name__ == "__main__":
    import runpy
    runpy.run_path(os.path.join(_ROOT, "qf_witness", "{cat}", "{name}.py"),
                   run_name="__main__")
else:
    # 동료 스크립트가 구경로로 `import {name}` 한 경우 — 본체 모듈로 자기 대체(완전 호환)
    import importlib
    sys.modules[__name__] = importlib.import_module("qf_witness.{cat}.{name}")
'''
