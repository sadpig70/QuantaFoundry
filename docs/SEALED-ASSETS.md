# Sealed Assets — the full catalogue (what's real, in detail)

> Moved from the README front page (2026-07-10) to keep the entry point approachable.
> This is the dense, complete inventory of what has been sealed or observed, with the
> honest qualifiers inline. Authoritative counts live in
> [`registry/REGISTRY-MANIFEST.json`](../registry/REGISTRY-MANIFEST.json); the guarantee-grade
> split lives in [`registry/SEMANTIC-GUARANTEES.json`](../registry/SEMANTIC-GUARANTEES.json).
> For claim-by-claim verification commands, see [`EVIDENCE-MAP.md`](EVIDENCE-MAP.md).
